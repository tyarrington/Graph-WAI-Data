...
Me = fetch(conn, commforM);
M = struct2table(Me);

% set the format of the incoming data to a table for easy access
setdbprefs('DataReturnFormat', 'structure');
Su = fetch(conn, commforS);
S = struct2table(Su);
...
if option == 1
    ...
    session_total = 0;
    for part=1:R
        if round(str2double(sub_num_subjects(part))) == round(id) && (strcmp(identifier, S_Identifiers(part)) || strcmp('NA', identifier))
    ...
    for index=1:W
        ...
        if round(str2double(sub_num(index))) == round(id) && (strcmp(identifier, identifiers(index)) || strcmp('NA', identifier))
