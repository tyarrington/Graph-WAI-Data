...
% September 2016 and named Measurements_Sept16.csv and Subject_Sept16.csv
Measurements=readtable('Measurements_Sept16.csv');
Subject_Info=readtable('Subject_Sept16.csv');

%Select data within Measurements_Sept16 from the PI you select and rename subtable
%Selected_data (has all data from PI)
...
% loops through each subject number
for Subject_Index=1:Number_Subjects
    % need to find  sessions=[1 2 3 5 6];
    %find data for subject of interest
    %Subject_Data_indices=find(Selected_Data.Sub_Number==Sub_Vector(Subject_Index));
    %Subject_Data=Selected_Data(Subject_Data_indices,:);
    Sessions=unique(Selected_Data.Session);
...
%suptitle(strcat('Data from',PI,'entries in WAI database'));
%title(strcat('Data from',PI,'entries in WAI database'));
